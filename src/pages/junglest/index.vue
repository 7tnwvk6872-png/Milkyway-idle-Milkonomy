...
function handleOriginLevelChange(source: "min" | "max") {
  const d = ldSearchData.value
  const min = d.minOriginLevel
  const max = d.maxOriginLevel
  if (min != null && max != null && min > max) {
    if (source === "min") {
      d.maxOriginLevel = min
    } else {
      d.minOriginLevel = max
    }
  }
  handleSearchLD()
}
          ...
          <template #header>
            <el-form class="rank-card" ref="ldSearchFormRef" :inline="true" :model="ldSearchData">
              ...
              <el-form-item :label="t('起始等级从')">
                <el-input-number style="width:80px" :min="1" :max="ldSearchData.maxOriginLevel || 20" v-model="ldSearchData.minOriginLevel" placeholder="1" clearable @change="handleOriginLevelChange('min')" controls-position="right" />&nbsp;{{ t('到') }}&nbsp;
